...
| `channels` | a Wii-menu paged grid: fixed `columns`×`rows` pages, greyed-out empty slots pad the last page, left/right page arrows in the gutters | `columns`, `rows`, `spacing`, `card.*` (as `grid`, label centred), `card.emptyFill`, `card.emptyBorder` |
