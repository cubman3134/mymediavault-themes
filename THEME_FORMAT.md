...
| `clock` | the current time as a 7-segment digital display + a small AM/PM | `onColor` (lit segments), `offColor` (unlit "ghost" segments), `ampmColor`, `ampmSize` (fraction of height), `thickness` (segment thickness, fraction of a digit's width) |
